...
frappe.views.GanttView = class GanttView extends frappe.views.ListView {
		...
		this.gantt = new Gantt(this.$result[0], this.tasks, {
			...
			on_progress_change: (task, progress) => {
				...
				if (progress_fieldname) {
					frappe.db.set_value(task.doctype, task.id, {
						[progress_fieldname]: setProgress(progress),
					}).then(() => {
						frappe.show_alert({message:`Прогресс в задача ${task.name} обновлен до ${setProgress(progress)}%`, indicator:'green'});
					});
				}
			},
			on_view_change: (mode) => {
	...
	get required_libs() {
		return [
			"/assets/tasks_app/js/node_modules/frappe-gantt/dist/frappe-gantt.css",
